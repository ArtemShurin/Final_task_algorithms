...
```bash
dijkstra_with_full_priority(graph, start, target, main_criterion)
```
Вход:
- Граф
...
```bash
find_best_route_by_criteria(graph, start_id, target_id, criterion_idx, id_to_name)
```
Вход:
Граф, ID городов
Индекс критерия
...
```bash
get_route_string(route, criterion_name)
```
Вход:
...
```bash
find_compromise_route(best_routes, priorities, id_to_name, graph, start_id, target_id)
```
Вход:
...
```bash
find_all_routes_dfs(graph, start, target, visited, path, all_routes, ...)
```
